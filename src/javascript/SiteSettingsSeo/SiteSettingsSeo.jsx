...
import {MuiThemeProvider, Toolbar, Typography, withStyles, withTheme} from '@material-ui/core';
import {NotificationProvider, SearchBar, SettingsLayout, theme, withNotifications} from '@jahia/react-material';
...
import Publication from './actions/Publication';
import Deletion from './actions/Deletion';
import PublishDeletion from './actions/PublishDeletion';
import Move from './actions/Move';
import AddVanityUrl from './actions/AddVanityUrl';
...
class SiteSettingsSeoAppComponent extends React.Component {
    constructor(props) {
        super(props);
        let {t} = this.props;
                ...
                call: (data, onSuccess, onError) => {
                    try {
                        props.vanityMutationsContext.update([data.urlPair.uuid],
                            data.defaultMapping === undefined ? undefined : data.defaultMapping.toString(),
                            data.active === undefined ? undefined : data.active.toString(),
                            data.language,
                            data.url)
                            .then(onSuccess)
                            .catch(ex => {
...
const SiteSettingsSeoApp = compose(
    withNotifications(),
    withTheme(),
    withStyles(styles),
    withVanityMutationContext(),
    translate()
)(SiteSettingsSeoAppComponent);
            ...
            <NotificationProvider>
                <ApolloProvider client={client({contextPath: props.dxContext.contextPath})}>
                    <I18nextProvider i18n={getI18n({
                        ...
                        ns: ['site-settings-seo', 'react-material'],
                        defaultNS: 'site-settings-seo',
                        namespaceResolvers: namespaceResolvers
                    })}
                    >
