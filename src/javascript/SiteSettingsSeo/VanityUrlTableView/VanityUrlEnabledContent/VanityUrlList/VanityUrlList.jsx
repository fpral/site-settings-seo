...
class VanityUrlListDefaultComponent extends React.Component {
    constructor(props) {
        super(props);
        this.state = {editLine: ''};
        ...
        return (
            <div>
                <div>
                    {urlPairs.length > 0 ? (
                        <Checkbox
                            className={(checkboxesDisplayed ? (expanded ? '' : classes.hidden) : (classes.hiddenOnHover)) + ' ' + classes.allCheckbox}
                                    ...
                                    return (
                                        ...
                                        >
                                            <TableCell padding="none"
                                                       className={(checkboxesDisplayed ? (expanded ? '' : classes.hidden) : (classes.hiddenOnHover)) + ' ' + classes.checkboxLeft}
                                            >
                                            ...
                                            <TableCell padding="none"
                                                       className={classInactive + ' ' + classes.tableCellTextInput}
                                            >
                                                <Editable value={url.url}
                                                          render={props => (this.props.filterText ? (
                                                              <HighlightText
                                                                      text={props.value}
                                                          ...
                                                          onEdit={(...params) => {
                                                              this.handleEdit(urlPair.uuid, ...params);
                                                          }}
                                                          onChange={(...params) => {
                                                              this.onMappingChanged(urlPair, actions, ...params);
                                                          }}
                                                />
                                            </TableCell>
                                            <TableCell padding="none"
                                                       className={classes.hiddenOnHover + ' ' + classInactive + ' ' + classes.tableCellActionsContainer}
                                            >
                                                {selection.length === 0 ? (
                                                    <span>
                                                        <ActionButton role="action-delete"
                                                                      className={classes.deleteAction}
                                                                      action={actions.deleteAction}
                                                                      data={[urlPair]}/>
                                                        <ActionButton role="action-move"
                                            ...
                                            <TableCell padding="none" className={classInactive}>
                                                <Checkbox className={url.default ? '' : classes.hiddenOnHover}
                                                          ...
                                                          onChange={event => actions.updateVanity.call({
                                                              urlPair: urlPair,
                                                              defaultMapping: event.target.checked
                                                          }, event)}
                                                />
                                            </TableCell>
                                            <TableCell padding="none"
                                                       className={classInactive + ' ' + classes.languageContainer}
                                            >
                                                <LanguageMenu languageCode={urlPair.default.language}
                                                              languages={languages}
                                            ...
                                            <TableCell padding="none"
                                                       ...
                                                       className={classes.publishArea}
                                            >
                                                {isPublished ? (
                                                    <Done classes={{root: classes.publishedCheck}}/>
                                                ) : (
                                                    <ActionButton role="action-publish"
...
class VanityUrlListLiveComponent extends React.Component {
    render() {
                        ...
                        <TableBody data-vud-table-body-live={contentUuid}>
                            {vanityUrls.map(urlPair => {
                                ...
                                if (url) {
                                    ...
                                    return (
                                        <TableRow key={urlPair.uuid}
                                                  className={classes.tableRow + ' ' + classInactive + ' ' + classes.vanityUrl + ' ' + classes.vanityUrlLive + ' ' + ((urlPair.default && !_.includes(defaultNotPublished, url)) ? '' : classes.missingDefaultCounterpart)}
                                        >
                                            <TableCell padding="none"
                                                       className={classInactive + ' ' + classes.liveVanityUrl}
                                            >
                                                {this.props.filterText ? (
                                                    ...
                                                    <Typography
                                                        className={classes.vanityURLText}
                                                    >{url.url}
                                                    </Typography>
                                                )}
                                            </TableCell>
                                            <TableCell padding="none"
                                                       className={classInactive + ' ' + classes.liveDefaultValue}
                                            >
                                                {url.default && <Star color={url.active ? 'secondary' : 'disabled'}/>}
                                            </TableCell>
                                            <TableCell padding="none"
                                                       className={classInactive + ' ' + classes.liveLanguage}
                                            >
                                                {url.language}
                                            </TableCell>
                                            <TableCell padding="none"
                                                       className={classInactive + ' ' + classes.actionButton}
                                                       style={{textAlign: 'center'}}
                                            >
                                                {url.editNode ?
    ...
    render() {
        ...
        return (
            <span className={this.props.classes.highlightTextContainer}>
                {parts.map((part, i) => (
                    <span key={i}
                          className={part.toLowerCase() === highlight.toLowerCase() ? this.props.classes.highlightText : ''}
                    >
                        {part}
                    </span>
                ))}
...
const VanityUrlListDefault = compose(
    withStyles(styles),
    withVanityMutationContext(),
    withNotifications(),
    translate()
)(VanityUrlListDefaultComponent);

const VanityUrlListLive = compose(
    withStyles(styles),
    translate()
)(VanityUrlListLiveComponent);

export {VanityUrlListLive, VanityUrlListDefault};
